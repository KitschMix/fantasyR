...
| 브랜치 | `main` |
| 마지막 커밋 | `a5b05a0 fix(sushi-go): 룰북 준수 — 108장 정정 + 김밥 동점 분배 룰 복원` |
...
### 2.6 스시고 룰북 재검증 + 108장/김밥 동점 룰 정정 (이번 세션, `a5b05a0`)
...
8. **sushi-go 카드 카운트 차이** ✅ **해결** (`a5b05a0`): 88장→108장 (원본 Gamewright 공식 그대로). 사시미 6→14, 김밥1 4→6, 김밥2 6→12, 젓가락 0→4(데코)
9. **sushi-go 김밥 동점 룰** ✅ **해결** (`a5b05a0`): `c9e37da`의 "모든 1등 동점자 6점씩" → 룰북대로 `Math.floor(6/N)` 분배로 복원. 1등 동점이면 2등 점수 없음 (룰북: "이 경우에는 두번째로 많은 사람은 점수를 얻지 못합니다")
